#!/usr/bin/env node
import {buildCreativePlan} from './creative-plan-lib.mjs';
...
try {
  ...
  if (durationSeconds === null || sceneCount === null) {
    throw new Error(
      '用法：project:plan -- <slug> --duration=<补全时长秒数> --scenes=<补全幕数> ' +
        ...
        '[--narration-seconds=<预计旁白秒数>] --rationale=<计算依据>',
    );
  }
  ...
  if (!['brief', 'concept-review'].includes(state.stage)) {
    throw new Error(
      `project:plan 只能在 brief 或 concept-review 阶段运行；当前为 ${state.stage}。`,
    );
  }
