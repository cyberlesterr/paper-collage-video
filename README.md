...
当前公开预览版本为 `0.5.0`；仓库正在验证尚未发布的 `0.7.0-dev.1`。新协议加入必需的节奏故事板、三种受限组合模式、本地关键帧、逐节拍视听 cue、真实组合证明和资产/组合双质量门；功能和协议仍可能在 `1.0.0` 前调整。
...
| `npm run project:composition-proof -- <slug>` | 用真实 v4 渲染器生成逐证明时刻全帧、关系裁切、调试图和 cue 表 |
...
每次输出都通过 `provider:run` 或 `provider:record` 写入 manifest v3，记录 provider、模型/任务 id、请求指纹、SHA-256、组合绑定、母版/派生成员和源家族指纹。相同 provider、模型、输入、设置和完整组合绑定才允许 `provider:reuse`；图像仍需在当前项目通过质量检查。完整契约见 [Provider Configuration](skills/make-paper-collage-video/references/providers.md)。
